...
export const productCatalogQuickFilters: ProductCatalogFilter[] = [
  { id: "all", label: "Semua", image: retortImage },
  { id: "sterilisator", label: "Retort", image: retortImage },
];
...
export function filterCatalogProducts(
  ...
  if (filters.color !== "all") {
    list = list.filter((product) => product.colorId === filters.color);
  }

  const availabilityFilters: CatalogProduct["availability"][] = [];
  if (filters.available) availabilityFilters.push("available");
  if (filters.limitedStock) availabilityFilters.push("limited");

  if (availabilityFilters.length === 1) {
    list = list.filter((product) =>
      availabilityFilters.includes(product.availability),
    );
  } else if (availabilityFilters.length === 2) {
    // Both checked — show all availability states.
  } else if (!filters.available && !filters.limitedStock) {
    list = [];
  }
