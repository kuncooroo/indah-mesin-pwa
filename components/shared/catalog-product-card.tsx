"use client";

import Link from "next/link";
import { Star } from "lucide-react";

import { FavoriteButton } from "@/components/shared/favorite-button";
import { ProductThumb } from "@/components/shared/product-thumb";
import type { CatalogProduct } from "@/lib/data/product-catalog";
import { favoriteProductId } from "@/lib/favorites-store";
import { cn } from "@/lib/utils";

type CatalogProductCardProps = {
  product: CatalogProduct;
  variant?: "grid" | "list";
};

const productCardClassName =
  "flex h-full flex-col overflow-hidden rounded-2xl border border-border-subtle bg-white shadow-sm transition-colors hover:border-primary";

export function CatalogProductCard({
  product,
  variant = "grid",
}: CatalogProductCardProps) {
  const favoriteId = favoriteProductId(product.slug, product.name);

  if (variant === "list") {
    return (
      <Link href={`/produk/${product.slug}`} className="block">
        <article className={cn(productCardClassName, "flex-row gap-3 p-3")}>
          <div className="relative size-[88px] shrink-0 overflow-hidden rounded-xl">
            <ProductThumb className="size-full" iconClassName="size-10" />
          </div>

          <div className="flex min-w-0 flex-1 flex-col justify-between py-0.5">
            <div>
              <h4 className="line-clamp-2 text-[13px] leading-snug font-bold text-on-surface">
                {product.name}
              </h4>
              <p className="mt-1 text-[12px] font-bold text-primary">Hubungi Kami</p>
            </div>

            <div className="mt-2 space-y-2">
              <div className="flex flex-wrap items-center gap-1.5">
                <span className="inline-flex items-center gap-0.5 rounded-md bg-[#fef9c3] px-1.5 py-0.5 text-[10px] font-semibold text-[#a16207]">
                  <Star className="size-3 fill-[#eab308] text-[#eab308]" />
                  {product.rating.toFixed(1)}
                </span>
                <span className="rounded-md bg-[#eff6ff] px-2 py-0.5 text-[10px] font-semibold text-primary">
                  {product.categoryBadge}
                </span>
              </div>

              <p className="text-[10px] text-on-surface-variant">{product.soldLabel}</p>
            </div>
          </div>
        </article>
      </Link>
    );
  }

  return (
    <Link href={`/produk/${product.slug}`} className="block h-full">
      <article className={productCardClassName}>
        <div className="relative aspect-[4/3] w-full shrink-0">
          <ProductThumb className="size-full" iconClassName="size-12" />
          <FavoriteButton
            product={{
              id: favoriteId,
              slug: product.slug,
              name: product.name,
              priceLabel: product.priceLabel,
              image: product.image,
            }}
            className="absolute top-2 right-2 z-10 text-primary/70"
            iconClassName="size-4"
          />
        </div>

        <div className="flex flex-1 flex-col space-y-2 p-3">
          <div className="flex-1">
            <h4 className="line-clamp-2 text-[13px] leading-snug font-bold text-on-surface">
              {product.name}
            </h4>
            <p className="mt-0.5 line-clamp-1 text-[10px] text-on-surface-variant">
              {product.categoryTag}
            </p>
          </div>

          <p className="text-[12px] font-bold text-primary">{product.priceLabel}</p>

          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center gap-0.5 text-[10px] font-semibold text-[#a16207]">
              <Star className="size-3 fill-[#eab308] text-[#eab308]" />
              {product.rating.toFixed(1)}
            </span>
            <span className="text-[10px] text-on-surface-variant">{product.soldLabel}</span>
          </div>
        </div>
      </article>
    </Link>
  );
}
