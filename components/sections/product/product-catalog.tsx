...
export function ProductCatalog({ initialFilter = "all" }: ProductCatalogProps) {
  ...
  const activeFilter = appliedFilters.category;

  function handleCategorySelect(categoryId: string) {
    setAppliedFilters((current) => ({ ...current, category: categoryId }));
  }
    ...
    <div className="pb-4">
      ...
      <section className="px-4 py-4">
        <div className="flex gap-6">
          {productCatalogQuickFilters.map((filter) => {
            ...
            return (
              ...
              >
                <div
                  className={cn(
                    "relative flex size-[56px] items-center justify-center overflow-hidden rounded-2xl border",
                    isActive
                      ? "border-[#bfdbfe] bg-[#eff6ff]"
                      : "border-border-subtle bg-white",
                  )}
                >
                  <RetortIcon className="size-7" />
                </div>
                <span
                  className={cn(
      ...
      <section className="px-4">
        <div className="mb-4 flex items-center justify-between gap-3">
          ...
          <button
            type="button"
            onClick={() => setShowFilter(true)}
            className="flex items-center gap-1 text-[12px] text-on-surface-variant"
          >
            <span>Sort:</span>
        ...
        <div className="grid grid-cols-2 gap-3">
          {paginatedProducts.map((product, index) => (
            <CatalogProductCard key={`${product.slug}-${index}`} product={product} />
          ))}
        </div>
